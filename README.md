...
For an opt-in project convention, `onekit-js/vite` can generate a `virtual:onekit/routes` module from a configured `src/app` or `src/pages` directory. It also exposes `fileRouteManifest`, `fileRouteLayouts`, and `fileRouteMiddleware` metadata for explicit application composition. The same plugin can validate leading `"use client"`/`"use server"` directives and reject client-to-server static imports at build time. These are **Experimental safety/composition contracts**; they do not provide a Server Components transport, automatic client code splitting, hidden middleware injection, or a complete Next.js-style project runtime. See [`VITE_ROUTE_COMPONENT_BOUNDARY.md`](docs/VITE_ROUTE_COMPONENT_BOUNDARY.md) for the configuration and limits.
