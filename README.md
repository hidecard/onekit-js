...
The latest V3.1.19 validation passes strict TypeScript checking, **35 Jest suites / 195 tests**, production build, declaration verification, clean package verification, real-browser smoke tests (**48 tests across Chromium, Firefox, WebKit, and Microsoft Edge**), including keyed-reorder identity, interaction, large-list timing, DOM-heavy timing, SSR hydration, and slot-heavy hydration/update/reorder coverage, and `git diff --check`. The current benchmark baseline is documented in [Performance Benchmarks](docs/PERFORMANCE_BENCHMARKS.md). Run `npm run benchmark` for runtime measurements, `npm run benchmark:memory` for forced-GC memory measurements, or `npm run test:browser` for the Playwright browser matrix. Browser performance budgets are versioned in `scripts/browser-performance-budgets.json`: the keyed-list hard limit is 150 ms, DOM-heavy is 200 ms, SSR hydration is 300 ms, slot-heavy hydration is 250 ms, slot-heavy updates are 300 ms, and slot-heavy reorders are 350 ms, with warnings at 80% of each limit. A hard-budget violation fails the CI browser job; warnings create GitHub Actions annotations and reports are retained as artifacts. Runtime and memory reports are emitted under `benchmark-results/`; browser traces and HTML reports are retained by CI on failure. The Vite plugin build may print non-fatal externalization notices for `node:fs`, `node:path`, and `typescript`; these are expected tooling/server externals and do not indicate a failed build.
