...
The latest V3.1.19 validation passes strict TypeScript checking, **35 Jest suites / 195 tests**, production build, declaration verification, clean package verification, real-browser smoke tests (**16 tests across Chromium, Firefox, WebKit, and Microsoft Edge**), and `git diff --check`. The current benchmark baseline is documented in [Performance Benchmarks](docs/PERFORMANCE_BENCHMARKS.md). Run `npm run benchmark` for runtime measurements, `npm run benchmark:memory` for forced-GC memory measurements, or `npm run test:browser` for the Playwright browser matrix. Runtime and memory reports are emitted under `benchmark-results/`; browser traces and HTML reports are retained by CI on failure. The Vite plugin build may print non-fatal externalization notices for `node:fs`, `node:path`, and `typescript`; these are expected tooling/server externals and do not indicate a failed build.
