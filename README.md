...
| Clean package verification | `npm run verify:package` | Clean install, CLI smoke check, and published-package export verification |
| API export-map verification | `npm run verify:api-contract` | Every advertised runtime target plus ESM/CJS subpath import resolution |

The published V3.1.19 validation passes strict TypeScript checking, **35 Jest suites / 195 tests**, production build, declaration verification, clean package verification, real-browser smoke tests (**48 tests across Chromium, Firefox, WebKit, and Microsoft Edge**), and a Chromium CDP lifecycle heap test (**1 passed, 3 intentionally skipped in non-Chromium projects**), including keyed-reorder identity, interaction, large-list timing, DOM-heavy timing, SSR hydration, slot-heavy hydration/update/reorder, and repeated mount/update/unmount coverage, and `git diff --check`. The current V3 continuation additionally validates secure route-data, cache tags, generated route declarations, explicit file-route composition, and transitive component-boundary diagnostics; its release count is finalized at the next release gate. The current benchmark baseline is documented in [Performance Benchmarks](docs/PERFORMANCE_BENCHMARKS.md). Run `npm run benchmark` for runtime measurements, `npm run benchmark:memory` for forced-GC memory measurements, `npm run test:browser` for the Playwright browser matrix, `npm run test:browser:heap` for lifecycle heap snapshots, or `npm run verify:api-contract` for the public ESM/CJS export-map contract. Browser budgets are versioned in `scripts/browser-performance-budgets.json`: keyed-list is 150 ms, DOM-heavy is 200 ms, SSR hydration is 300 ms, slot-heavy hydration is 250 ms, slot-heavy updates are 300 ms, slot-heavy reorders are 350 ms, and lifecycle retained post-GC heap growth is 8 MiB, with workload-specific warnings. The heap test writes before/after Chromium `.heapsnapshot` artifacts and fails when retained heap growth exceeds its hard budget. GitHub Actions runs this check through `.github/workflows/browser-heap-snapshot.yml` on V3 pushes and pull requests, with a manual `workflow_dispatch` option and 14-day diagnostic artifacts. The workflow also restores cache-backed lifecycle heap history, compares the current retained-heap result with the previous Chromium run, and emits a warning for regressions above 25%; the 8 MiB retained-heap budget remains the hard gate. A hard-budget violation fails the CI browser job; warnings create GitHub Actions annotations and reports are retained as artifacts. Runtime and memory reports are emitted under `benchmark-results/`; browser traces and HTML reports are retained by CI on failure. The Vite plugin build may print non-fatal externalization notices for `node:fs`, `node:path`, and `typescript`; these are expected tooling/server externals and do not indicate a failed build.
...
For an opt-in project convention, `onekit-js/vite` can generate a `virtual:onekit/routes` module from a configured `src/app` or `src/pages` directory. It also exposes `fileRouteManifest`, `fileRouteEntries`, `fileRoutePaths`, `fileRouteLayouts`, `fileRouteMiddleware`, and explicit association metadata. The declaration-only companion `virtual:onekit/routes.d.ts` provides the literal `FileRoutePath` union and `FileRouteParams<Path>` helper for typed links. `composeFileRouteInfrastructure()` can resolve page modules with ordered directory-scoped layout and middleware values when the application wants explicit composition. The same plugin can validate leading `"use client"`/`"use server"` directives and reject client-to-server static imports at build time. These are **Experimental safety/composition contracts**; they do not provide a Server Components transport, automatic client code splitting, hidden middleware injection, or a complete Next.js-style project runtime. See [`VITE_ROUTE_COMPONENT_BOUNDARY.md`](docs/VITE_ROUTE_COMPONENT_BOUNDARY.md) for the configuration and limits.
