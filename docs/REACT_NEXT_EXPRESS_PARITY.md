...
**OneKit implication:** OneKit has SSR, hydration, route loaders, VDOM, and explicit render hooks, but it does not yet have a Server Component module graph, a server/client directive boundary, a Flight-like transport, or a compiler/bundler contract for server components.
...
**OneKit implication:** OneKit already covers route matching, nested layouts, lazy routes, loaders, prefetch, metadata, manifests, SSR/hydration, and an optional `createRouterView()`. The major gaps are the project-level file-system convention, integrated server/client component compilation, server functions/actions, streaming route payloads, and a unified cache/revalidation/deployment contract.
...
| Server/client component boundary | Not implemented as a compiler/runtime contract | Server Components and Client Components | Integrated App Router model | Not applicable | **P0 gap** if Next-like parity is a goal. |
| File-system routes | `createFileRoutes()` with `index/page`, dynamic, catch-all, optional catch-all, and route-group normalization; experimental route metadata | External/tooling choice | Core `app` convention | Not applicable | **P1 gap:** full project plugin, generated typed route pipeline, middleware/layout file conventions, and stable deployment integration. |
