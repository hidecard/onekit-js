...
| Router and route rendering | **Implemented/experimental mix** | Streamed SSR route integration, generated fully typed route modules, runtime layout/middleware composition, and deployment adapter policy. |
...
The practical V3 file-route plugin now provides project-level discovery, configurable extensions, duplicate-path diagnostics, generated metadata, and explicit layout/middleware associations. QueryClient and route loaders share tagged invalidation and `revalidate` freshness options, and SSR query handoff preserves tags. The next work is generated fully typed route declarations, runtime composition utilities, prerendering, adapter integration, Server Functions/actions, deployment policy, and observability.
...
| Route output | Deterministic manifest, conflict diagnostics, ordinary route objects, configurable extensions, and explicit layout/middleware metadata. | Next.js connects conventions to layouts, loading/error files, metadata, route handlers, prerendering, and navigation payloads. |
