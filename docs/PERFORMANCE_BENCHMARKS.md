...
These measurements do not include browser DOM node memory, layout/style cost, event-listener memory, V8 code space, RSS, allocator fragmentation, or comparisons against React/Vue/Svelte. The real-browser suite now covers DOM reconciliation, keyed list reorder timing, large server-rendered hydration timing, slot-heavy projection hydration, slot-heavy content updates, and slot-heavy keyed reorders with enforced budgets. Historical trend storage and browser heap snapshots after repeated mount/unmount cycles remain follow-up work.
