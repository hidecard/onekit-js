...
| File-based route helpers, generated route metadata, and opt-in Vite virtual route module | Experimental | Bundler-safe helpers cover configurable extensions, index/page, dynamic, catch-all, optional catch-all, route groups, deterministic conflict diagnostics, layouts, middleware metadata, explicit associations, and typed URL generation. Generated fully typed route declarations and runtime composition remain outside the stable framework contract. |
