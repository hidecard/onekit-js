...
| Streaming SSR, progressive boundary scheduling, adapter `scheduleBoundary` hooks, bounded `maxConcurrentBoundaries`, inert route-data/RSC payload script handoff, and `readStreamingPayloads()` | Experimental | Additive adapter contract with cancellation, error, payload escaping, client extraction, concurrency-limit, and back-pressure-control tests; platform queue/back-pressure behavior remains adapter-dependent. Route-data validation remains in `ssr-data`; RSC-style records remain a separate Experimental protocol. |
...
| `createISRRenderer()`, `ISRCache`, `ISRCacheLock`, serialized KV cache adapters, lifecycle events, and tag-aware page revalidation | Experimental | Fresh-hit, stale-while-revalidate, missing-page regeneration, single-flight concurrency, failure retention, tag invalidation, QueryClient coordination, memory-cache behavior, serialized KV namespacing/validation, adapter-owned lease release, scheduling hooks, and diagnostics isolation are tested. Durable storage semantics, distributed lock correctness/expiry, eviction, authorization, webhook verification, and deployment headers remain application-owned. |
| Official provider integrations: `@onekit-js/redis`, `@onekit-js/deno-kv`, `@onekit-js/vercel-global-config` | Experimental | Redis storage/lease, Deno KV storage/atomic lease, and injected Vercel Global Config read-client contracts are tested. Vendor SDK versions, connection/retry policy, secrets, deployment manifests, outage handling, and Vercel Global Config's read-only/non-ISR nature remain explicit application responsibilities. |
| Experimental RSC-style records, client references, `encodeRSCFlight()`/`decodeRSCFlight()`, `resolveRSCFlight()`, and `createRSCFlightStream()` | Experimental | Bounded versioned records, serializable-model checks, explicit resolver behavior, malformed-input rejection, prototype-pollution-key rejection, abort-aware streaming, and inert SSR script transport are tested. This is not React Flight/RSC compatibility and does not provide compiler transforms, bundle splitting, component execution, Server Functions, or navigation semantics. |
| Edge deployment adapter, `detectEdgeRuntime()`, `assertEdgeRuntime()`, `createEdgeRequestContext()`, and `EdgeHandler.schedule()` | Experimental | Fetch-native direct delegation, capability failure checks, streamed response identity, optional Web Crypto requirements, `waitUntil()` forwarding, and scheduled-task bridging are tested. Vendor bindings, environment typing, CDN headers, durable ISR storage, and platform manifests remain outside the core package. |
