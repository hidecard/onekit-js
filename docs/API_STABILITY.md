...
| File-based route helpers and generated route metadata | Experimental | Bundler-safe helpers cover index/page, dynamic, catch-all, optional catch-all, route groups, and typed URL generation. The full project-level file-system convention and generated typed route pipeline are not yet a stable framework contract. |
