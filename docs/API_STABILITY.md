...
| File-based route helpers, generated route metadata, declaration-only route-path types, explicit composition helper, and opt-in Vite virtual route module | Experimental | Bundler-safe helpers cover configurable extensions, index/page, dynamic, catch-all, optional catch-all, route groups, deterministic literal/ambiguous-pattern conflict diagnostics, layouts, middleware metadata, explicit associations, `FileRoutePath`/`FileRouteParams`, and `composeFileRouteInfrastructure()`. Automatic runtime composition and prerendering remain outside the stable framework contract. |
...
The published `3.1.19` release records **195 Jest tests**. The current continuation working tree adds secure route-data, cache tags, file-route conflicts/associations, generated declaration-module coverage, explicit composition, and transitive component-boundary coverage; the final release count is regenerated after the complete validation matrix and release artifacts are updated. Release-facing historical counts remain tied to the version they describe until the next package release is cut.
