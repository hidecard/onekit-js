...
| Streaming SSR, progressive boundary scheduling, adapter `scheduleBoundary` hooks, inert route-data/RSC payload script handoff, and `readStreamingPayloads()` | Experimental | Additive adapter contract with cancellation, error, payload escaping, and client extraction tests; queue and back-pressure behavior remains platform-dependent. Route-data validation remains in `ssr-data`; RSC-style records remain a separate Experimental protocol. |
