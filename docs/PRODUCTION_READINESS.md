...
| M6 | Backend foundation | `createApi()`, `app.resource()`, typed body helpers, route/middleware contracts, validation, DI context, safe defaults, `createNodeHandler()`, authentication/authorization contracts, session/token provider adapters, typed database adapter context, bounded rate limiting with a portable store contract, and adapter-neutral `Request`/`Response` behavior are documented and tested. Concrete distributed store implementations, database integrations, identity storage, and decorator modules remain follow-up work. |
