...
| VDOM/JSX | VDOM/JSX helpers with keyed element reconciliation, fragments, refs, controlled input properties, and event cleanup | The keyed reconciliation milestone is implemented and tested, including stable keyed-node moves across mixed keyed/unkeyed insertions, cleanup of removed keyed subtrees, and keyed function-component identity with prop updates. Stateful component-instance reconciliation, richer ref semantics, and hydration parity remain follow-up work. |
...
The current options-style component API is useful, and the renderer now documents a consistent first update model: VDOM reconciliation for element children and keyed function components, with template patching remaining a separate path. Keyed list moves, keyed function-component prop updates, refs, event listener cleanup, error boundaries, and unmount cleanup have baseline coverage. Remaining parity work includes stateful component-instance reconciliation, richer ref semantics, non-string slots, and hydration integration.
