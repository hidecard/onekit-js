...
Remaining SSR work includes advanced async concurrency, serialized loader state, and a larger browser compatibility matrix. Promise roots/children are now scheduled deterministically in source order, with regression coverage for original render failures and AbortSignal cancellation.
