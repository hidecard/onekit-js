...
| Components | Options-style components, templates, lifecycle hooks, mount/unmount | Suitable for small production features. Component lifecycle cleanup and template event ownership are covered; broader React-like reconciliation, refs, controlled inputs, and component-level update semantics remain incremental work. |
| VDOM/JSX | VDOM/JSX helpers with keyed element and component reconciliation, fragments, refs, controlled input properties, and event cleanup | Keyed element reconciliation, keyed function-component identity, stateful component-instance reuse, prop updates, stable root DOM identity, lifecycle continuity, and unmount cleanup are implemented and tested. Richer ref semantics and hydration parity remain follow-up work. |
| SSR | String rendering, streaming utilities, request-scoped context, hydration helpers, metadata helpers, progressive boundary chunks, client continuation, and adapter-level boundary scheduling | M4 baseline complete: escaping, nested context propagation, metadata safety, hydration mismatch diagnostics, listener disposal, and boundary primitives are tested. Progressive boundaries emit visible fallback/content chunks with import-safe client continuation, streaming preserves original async render errors and supports AbortSignal cancellation, and adapters can now provide an optional `scheduleBoundary(task, boundary)` hook for queueing or back-pressure policies. Advanced async concurrency and platform-specific stream integration remain future work. |
...
Remaining SSR work includes advanced async concurrency, trusted application transport for loader state, stronger queue/back-pressure cancellation semantics, and a larger browser compatibility matrix. Adapter-level scheduling is now available as an optional hook and covered by regression tests; progressive fallback/content boundaries and client continuation are also covered, alongside deterministic Promise scheduling, original render failures, and AbortSignal cancellation.

### 5. Improve component rendering architecture

The current options-style component API is useful, and the renderer now documents a consistent first update model: VDOM reconciliation for element children, keyed function components, and keyed stateful component instances, with template patching remaining a separate path. Keyed list moves, keyed function-component prop updates, state preservation across keyed moves, stable component root identity, lifecycle continuity, refs, event listener cleanup, error boundaries, and unmount cleanup have baseline coverage. Remaining parity work includes richer ref semantics, non-string slots, and hydration integration.
