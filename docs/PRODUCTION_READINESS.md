...
| Backend | Fetch-compatible `createApi()`/`createServerApp()`, Node HTTP bridge, route methods, middleware, validation, DI context, safe responses, CORS, request IDs, authentication, authorization, and bounded rate limiting | Beginner-friendly backend foundation is implemented and tested. `createNodeHandler()` covers Node HTTP; `app.handle(request)` remains the adapter-neutral serverless/edge contract. Distributed rate-limit stores, database contracts, token/session providers, and decorator-based modules remain future work. |
...
OneKit now has an adapter-neutral backend foundation plus an official lightweight Node HTTP bridge. The beginner path is `createApi()` with concise `context.ok()`, `context.json()`, `context.text()`, and `context.fail()` helpers. The lower-level `createServerApp()` API supports ordered middleware, decoded route params, query values, JSON validation, request state, and the existing dependency injector. `createNodeHandler()` connects the app to Node HTTP, while `app.handle(request)` remains suitable for serverless and edge adapters. Authentication/authorization contracts and bounded in-memory rate limiting are available; distributed rate-limit stores, database adapter guidance, token/session providers, and server-only secret handling remain application responsibilities.
...
| M6 | Backend foundation | `createApi()`, route/middleware contracts, validation, DI context, safe defaults, `createNodeHandler()`, authentication/authorization contracts, bounded rate limiting, and adapter-neutral `Request`/`Response` behavior are documented and tested. Distributed stores, database integrations, and decorator modules remain follow-up work. |
