...
| M6 | Backend foundation | `createApi()`, `app.resource()`, typed body helpers, route/middleware contracts, validation, DI context, safe defaults, `createNodeHandler()`, authentication/authorization contracts, session/token provider adapters, typed database adapter context, bounded rate limiting with a portable store contract, typed `ServerError` responses, resilient error hooks, the optional full-stack CLI starter, `createServerData()` with injectable caching, decorator-free module/controller composition, the driver-injected optional `createSQLiteAdapter()`, and adapter-neutral `Request`/`Response` behavior are documented and tested. Concrete distributed cache/store implementations, PostgreSQL integrations, identity storage, decorator metadata, and framework-owned persistence remain follow-up work. |
