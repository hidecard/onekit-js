...
function topicCard(topic: Topic) {
  return `<article class="topic-card" id="${topic.id}"><div class="topic-meta"><span>${topic.area}</span><span>V3.1.16</span></div><h3>${escapeHtml(topic.title)}</h3><p>${escapeHtml(topic.summary)}</p><code>${escapeHtml(topic.signature)}</code><pre>${escapeHtml(topic.example)}</pre></article>`;
}

function render() {
  const topicsHtml = visibleTopics.value.map(topicCard).join("");
  app.innerHTML = `<div class="manual-shell">
    <header class="topbar"><a class="brand" href="#top"><span class="brand-mark"><i></i><b>+</b></span><span>ONEKIT <em>JS</em></span></a><div class="version">V3.1.16 / FIELD MANUAL</div><button class="menu-button" data-action="menu" aria-label="Toggle navigation">☰</button></header>
    ...
    </main><footer class="footer">ONEKIT JS / V3.1.16 <span>Documentation example built with reactive(), computed(), effect(), and nextTick().</span></footer></div>`;
}
