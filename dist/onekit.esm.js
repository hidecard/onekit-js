...
function patchChildren(parent, nextChildren, previousChildren) {
    ...
    const entries = previousChildren.map((vnode, index) => ({
        vnode,
    ...
    nextChildren.forEach(nextChild => {
        ...
        if (nextKey !== undefined) {
            ...
            while (unkeyedIndex < unkeyed.length && unkeyed[unkeyedIndex].used)
                unkeyedIndex += 1;
...
function patchChildrenLegacy(parent, nextChildren, previousChildren) {
    const used = new Set();
 ...
 */
function createFileRoutes(modules, options = {}) {
...
class QueryClient {
    ...
    invalidate(key) {
        const records = key === undefined
            ...
            : [this.records.get(normalizeKey(key))].filter(Boolean);
        for (const record of records) {
    ...
    async mutate(variables, options) {
        ...
        if (optimistic)
            this.setData(optimistic.key, optimistic.update(previous, variables));
        let data;
        let error;
        try {
            const retry = options.retry;
                ...
                try {
                    ...
                    if (optimistic) {
                        const rolledBack = optimistic.rollback
                            ? optimistic.rollback(this.getData(optimistic.key), context, variables)
                            : previous;
...
function hydrate(rootElement, vnode, options = {}) {
    const mismatches = [];
    const cleanups = [];
...
function clientOnly(callback) {
    return isClientRuntime() ? callback() : undefined;
}
/** Assert that the current code is executing on the server. */
function assertServer(message = 'This API is only available during server rendering.') {
    if (!isServerRuntime())
        throw new Error(message);
}
/** Assert that the current code is executing in a browser. */
function assertClient(message = 'This API is only available in a browser.') {
    if (!isClientRuntime())
        throw new Error(message);
}
