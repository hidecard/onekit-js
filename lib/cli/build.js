...
export async function build(options = {}) {
  ...
  if (!await exists(entryPath)) throw new Error(`Entry point "${entryPoint}" not found.`);
  ...
  const outputs = [
    ...
    { file: path.join(outputDir, `${base}.cjs.js`), format: 'cjs', sourcemap: true },
  ...
  await bundle.close();
  await writeFile(path.join(outputDir, 'package.json'), JSON.stringify({ ...pkg, main: `${base}.cjs.js`, module: `${base}.esm.js`, browser: `${base}.js`, scripts: undefined, devDependencies: undefined }, null, 2) + '\n');
