...
const runProjectBuild = (cwd, outputDir) => new Promise((resolve, reject) => {
  const args = ['run', 'build'];
  if (outputDir) args.push('--', '--outDir', outputDir);
  const child = spawn(npmCommand, args, { cwd, stdio: 'inherit', shell: false, windowsHide: false });
...
export async function build(options = {}) {
  ...
  if (!await exists(entryPath)) {
    if (pkg.scripts?.build) {
      ...
      await runProjectBuild(process.cwd(), output === 'dist' ? undefined : outputDir);
      return;
    }
