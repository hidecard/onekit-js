...
const DEVELOPER_CREDIT = 'Developed By Arkar Yan ( H!D3_C4rD )';
...
const fullStackServer = ({ packageName }) => `import { createServer } from 'node:http';
import { createApi, createNodeHandler } from 'onekit-js';

const app = createApi();
app.get('/api/health', ({ ok }) => ok({ ok: true, service: ${JSON.stringify(packageName)} }));

const port = Number(process.env.PORT || 3001);
const server = createServer(createNodeHandler(app));
await app.start();
server.listen(port, () => console.log(\`OneKit API listening on http://localhost:\${port}\`));
...
<template>
  <main class="shell">
    ...
    <section class="hero" aria-labelledby="welcome-title"><div class="hero-copy"><p class="eyebrow"><span class="status-dot"></span> Ready to build</p><h1 id="welcome-title">Ship your next idea <span>faster.</span></h1><p class="lede">A production-minded starter for ${packageName}, with fast feedback, fine-grained reactivity, and a clean path from first component to deploy.</p><div class="hero-actions"><a class="primary-link" href="https://github.com/hidecard/onekit-js" target="_blank" rel="noreferrer">Explore OneKit <span aria-hidden="true">↗</span></a><a class="text-link" href="https://github.com/hidecard/onekit-js#readme" target="_blank" rel="noreferrer">Read the docs</a></div></div><div class="orb" aria-hidden="true"><span>OK</span></div></section>
...
export async function createApp(appName, options = {}) {
  ...
  const packageJson = {
    ...
    scripts: {
      ...
      test: 'node --test',
      ...(fullStack ? { 'dev:server': 'node --watch server.mjs', start: 'node server.mjs' } : {}),
      ...(isTypeScript ? { 'type-check': 'tsc --noEmit' } : {})
    },
    // Keep the starter installable before the next framework patch is published;
    // the caret range resolves the newest compatible V3 release automatically.
    dependencies: { 'onekit-js': '^3.1.18' },
...
<html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><meta name="description" content="${packageName} — a OneKit JS V3 starter project"><meta name="theme-color" content="#090d1c"><title>${packageName} · OneKit JS</title></head>
<body><main id="app"></main><script type="module" src="/src/main.${isTypeScript ? 'ts' : 'js'}"></script></body></html>\n`);
  ...
  if (fullStack) {
    await writeFile(path.join(appPath, 'server.mjs'), fullStackServer({ packageName }));
    await writeFile(path.join(appPath, '.env.example'), fullStackEnv);
  }
...
`);
  if (isTypeScript) {
    ...
    await writeFile(path.join(appPath, 'src', 'vite-env.d.ts'), '/// <reference types="vite/client" />\n');
  ...
  await writeFile(path.join(appPath, 'README.md'), `# ${packageName}\n\nA modern OneKit JS V3${fullStack ? ' full-stack' : ''} starter with a responsive reactive playground and a fast Vite-style workflow.\n\n> Developed By Arkar Yan ( H!D3_C4rD )\n\nThis project was created with the OneKit JS starter CLI. It includes a Vite development server, a reactive .okjs component, and a production build workflow.\n\n## Development\n\n~~~bash\nnpm install\nonekit dev\n~~~\n\nOpen the local URL printed by Vite. Edit src/App.okjs to change the UI; Vite HMR preserves component state while you work. The equivalent npm script is npm run dev.\n\n## Production build and tests\n\n~~~bash\nonekit build\nonekit preview\nonekit test\n~~~\n\n${fullStack ? `
## Full-stack API

This starter also includes a Fetch-compatible API in \`server.mjs\`.

~~~bash
npm run dev:server
~~~

The example endpoint is \`GET /api/health\`. Set \`PORT\` in the environment before starting. The server uses OneKit's adapter-neutral \`createApi()\` core and the Node HTTP bridge; deploy it behind your preferred Node, serverless, or edge adapter.

\`npm start\` runs the API in production mode.

` : ''}For this Vite-style starter, onekit build delegates to npm run build, which runs vite build. Run onekit preview after a successful build; the CLI requires the dist/ directory. The equivalent direct commands are npm run build, npm run preview, and npm test.\n`);
