...
export default {
  ...
  data: () => ({ count: 0, step: 1 }),
  methods: {
    ${isTypeScript ? 'increment(this: any)' : 'increment()'} { this.state.count += Number(this.state.step) || 1; this.update(); },
    ${isTypeScript ? 'decrement(this: any)' : 'decrement()'} { this.state.count -= Number(this.state.step) || 1; this.update(); },
    ${isTypeScript ? 'reset(this: any)' : 'reset()'} { this.state.count = 0; this.update(); },
  },
};
</script>
<template>
