...
export default {
  ...
  data: () => ({ count: 0, step: 1 }),
  methods: {
    increment(this${isTypeScript ? ': any' : ''}) { this.state.count += Number(this.state.step) || 1; this.update(); },
    decrement(this${isTypeScript ? ': any' : ''}) { this.state.count -= Number(this.state.step) || 1; this.update(); },
    reset(this${isTypeScript ? ': any' : ''}) { this.state.count = 0; this.update(); },
  },
};
</script>
<template>
